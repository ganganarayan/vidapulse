// Country dial-code list for the phone/WhatsApp field on the signup form.
// Each entry: { code: ISO-3166-1 alpha-2, name, dial: E.164 calling code (no +) }.
// Used by Register.jsx — the selected country's `dial` is prefixed to the
// national number on submit. The default country is chosen from the user's IP
// via GET /api/geo (geoip-lite), falling back to India (IN) on any failure.
//
// Sorted alphabetically by name. No flag emojis on purpose: on Windows they
// render as bare letters, and a leading emoji breaks native <select> type-ahead
// (typing the first letters of a country name to jump to it).
...
// Quick lookup by ISO code → dial. Returns null when the code isn't listed.
const _byCode = COUNTRIES.reduce((m, c) => { m[c.code] = c; return m; }, {});

export function dialForCountry(code) {
  return _byCode[String(code || '').toUpperCase()]?.dial ?? null;
}
