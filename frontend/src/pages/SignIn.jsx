 ...
 *   Step 1 — enter email → check /auth/email-status
 *     • Google account  → redirect through Google OAuth (login_hint set)
 *     • Existing password account → Step 2a: password field → login
 *     • Not found → Step 2b: register form (create account)
 *
 * The Register path fires the StartTrial pixel.
 * Deactivated accounts are handed off to /login (restore flow lives there).
 * Never auto-creates an account on a sign-in attempt — only the Register
 * sub-form does.
 */
export default function SignIn() {
  ...
  useEffect(() => {
    api.get('/auth/providers')
      .then(r => setGoogleEnabled(!!r.data.google))
      .catch(() => {});
  }, []);
  ...
  async function handleEmailSubmit(e) {
    ...
    setChecking(true);
    try {
      const { data } = await api.get(`/auth/email-status?email=${encodeURIComponent(trimmed)}`);

      if (data.google) {
        // Existing Google account → send through OAuth
        window.location.href = `/api/auth/oauth/google?login_hint=${encodeURIComponent(trimmed)}`;
        return;
      }
      if (data.exists) {
        setStep('password');
  ...
  async function handleSignIn(e) {
    ...
    try {
      ...
      setError(err.response?.data?.error ?? err.response?.data?.message ?? 'Incorrect password. Try again.');
      setLoading(false);
    }
  }
  ...
  const strength = [password.length >= 8, password.length >= 12, /[A-Z]/.test(password), /[0-9]/.test(password), /[^A-Za-z0-9]/.test(password)].filter(Boolean).length;
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8">
          ...
          {step === 'email' && (
            <>
              <h1 className="text-xl font-bold text-gray-50 mb-1">Sign in</h1>
              <p className="text-sm text-gray-400 mb-6">Enter your email to continue.</p>
          ...
          {/* ── STEP 2a: password (existing account) ───────────── */}
          {step === 'password' && (
            <>
              <button
              ...
              <p className="text-sm text-gray-400 mb-6 truncate">
                Signing in as <span className="text-gray-200">{email}</span>
              </p>
          ...
          {step === 'register' && (
            <>
              ...
              <p className="text-sm text-gray-400 mb-1">
                No account found for <span className="text-gray-200">{email}</span>.
              </p>
              ...
              <form onSubmit={handleRegister} className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={loading}
                    autoComplete="new-password"
                    className={`w-full border focus:outline-none text-gray-100 placeholder-gray-500
                                text-sm rounded-lg px-3 py-2.5 transition-colors
                                ${confirm && confirm !== password
                                  ? 'bg-gray-700 border-red-500/60 focus:border-red-500'
        ...
        {step === 'email' && (
          <p className="text-center text-sm text-gray-500 mt-5">
            Have a magic link?{' '}
            <span className="text-gray-400">Check your email or WhatsApp.</span>
          </p>
        )}
